...
//procura código na árvore a partir da raíz (representado por String com 0s e 1s): procura codigo inteiro
...
//procura codigo na árvore, a partir do nó actual (representado por String com 0s e 1s)
//devolve índice no alfabeto, se encontrou; -1 se não encontrou; -2 se é prefixo de codigo existente;
int findNode(HuffmanTree *hft, char *s, HFNode *cur, short verbose)
{
	...
	{
		if (pos == -1)
			printf("Codigo '%s' não encontrado!!!\n", s);
		else if (pos == -2)
			printf("Cdigo '%s' não encontrado, mas prefixo!!!\n", s);
		else
			printf("Cdigo '%s' corresponde à posição %d do alfabeto\n", s, pos);
	}
